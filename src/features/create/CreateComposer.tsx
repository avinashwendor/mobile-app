...
type StoryFilterId = 'original' | 'warm' | 'cool' | 'mono' | 'sunset';

const STORY_FILTERS: { id: StoryFilterId; label: string; colors: readonly [string, string, ...string[]]; overlayOpacity: number }[] = [
  { id: 'original', label: 'Original', colors: ['transparent', 'transparent'], overlayOpacity: 0 },
  { id: 'warm', label: 'Warm', colors: ['#FF8C421A', '#FFB34766'], overlayOpacity: 0.28 },
  { id: 'cool', label: 'Cool', colors: ['#3B82F61F', '#06B6D466'], overlayOpacity: 0.24 },
  { id: 'mono', label: 'Mono', colors: ['#11182733', '#D1D5DB66'], overlayOpacity: 0.22 },
  { id: 'sunset', label: 'Sunset', colors: ['#FB71851F', '#F9731666'], overlayOpacity: 0.3 },
];

const isCreateMode = (value: string | undefined): value is CreateMode => (
  value === 'post' || value === 'story' || value === 'reel'
);
...
const MODE_META: Record<CreateMode, {
  ...
  story: {
    ...
    hint: 'Multiple photos or videos, max 60s per clip',
    allowsMultiple: true,
    mediaTypes: ImagePicker.MediaTypeOptions.All,
    maxDuration: 60,
    aspectRatio: 9 / 16,
...
export default function CreateScreen() {
  ...
  const params = useLocalSearchParams<{ mode?: string; openPicker?: string }>();
  const insets = useSafeAreaInsets();
  const { colors, isDark } = useTheme();
  const requestedMode = Array.isArray(params.mode) ? params.mode[0] : params.mode;
  const openPicker = Array.isArray(params.openPicker) ? params.openPicker[0] : params.openPicker;

  const [mode, setMode] = useState<CreateMode>(isCreateMode(requestedMode) ? requestedMode : 'post');
  ...
  const [storyFilterId, setStoryFilterId] = useState<StoryFilterId>('original');
  ...
  const handleModeChange = useCallback((next: CreateMode) => {
    ...
    setStoryFilterId('original');
  }, [mode]);

  useEffect(() => {
    if (!isCreateMode(requestedMode) || requestedMode === mode) return;
  ...
  const pickMedia = useCallback(async () => {
    try {
      ...
      if (permission.status !== 'granted') {
        Alert.alert(
          'Permission needed',
          `We need access to your photos to pick media for your ${meta.label.toLowerCase()}.`,
        );
        return;
      }
      ...
      if (mode === 'story' && assets.some((asset) => asset.type === 'video' && (asset.duration ?? 0) > 60_000)) {
        Alert.alert(
          '60-second maximum',
          'Story videos must be 60 seconds or less in this build. If your device shows a trim option in the picker, choose a 60-second clip there first.',
        );
        return;
      }
  ...
  const captureMedia = useCallback(async () => {
    try {
      ...
      if (mode === 'story' && asset.type === 'video' && (asset.duration ?? 0) > 60_000) {
        Alert.alert('60-second maximum', 'Story recordings stop at 60 seconds. Please try again.');
        return;
      }
  ...
  }, [media.length, meta, mode]);

  useEffect(() => {
    if (mode !== 'story' || openPicker !== '1' || autoPickerOpenedRef.current || media.length > 0) return;
    autoPickerOpenedRef.current = true;
    pickMedia();
  }, [media.length, mode, openPicker, pickMedia]);
        ...
        if (mode === 'post') {
          ...
          for (const asset of media) {
            await storyApi.createStory({
              ...
              filterId: storyFilterId === 'original' ? undefined : storyFilterId,
            });
          }
        }
  ...
  }, [canPublish, caption, captionTokens, commentsEnabled, likesVisible, location, media, mode, router, storyFilterId, visibility]);
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      {mode === 'story' ? (
        <StoryToolsBar
          selectedFilterId={storyFilterId}
          onSelectFilter={setStoryFilterId}
...
function MediaStage({
  ...
  const selectedFilter = STORY_FILTERS.find((filter) => filter.id === storyFilterId) ?? STORY_FILTERS[0];
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={{ width: stageWidth, height: stageHeight }}>
              ...
              {mode === 'story' && selectedFilter.overlayOpacity > 0 ? (
                <LinearGradient
                  pointerEvents="none"
                  colors={[...selectedFilter.colors]}
                  style={[StyleSheet.absoluteFill, { opacity: selectedFilter.overlayOpacity }]}
                />
              ) : null}
              {item.type === 'video' ? (
...
function StoryToolsBar({
  ...
  selectedFilterId: StoryFilterId;
  onSelectFilter: (filterId: StoryFilterId) => void;
  selectedCount: number;
  colors: any;
}) {
  return (
    <View style={styles.storyToolsSection}>
      <View style={styles.storyToolsHeader}>
        <Text style={[styles.storyToolsTitle, { color: colors.text }]}>Story tools</Text>
        <View style={[styles.storyToolsCount, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Ionicons name="images-outline" size={14} color={colors.textSecondary} />
          <Text style={[styles.storyToolsCountText, { color: colors.textSecondary }]}>
            {selectedCount > 0 ? `${selectedCount} selected` : 'Multi-select ready'}
          </Text>
        </View>
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storyFilterRow}>
            ...
            <Pressable
              ...
              style={[
                styles.storyFilterChip,
                {
                  ...
                  backgroundColor: active ? 'rgba(108,92,231,0.1)' : colors.surface,
                },
              ]}
            >
              <LinearGradient colors={[...filter.colors]} style={styles.storyFilterPreview} />
              <Text style={[styles.storyFilterLabel, { color: active ? Colors.primary : colors.text }]}>{filter.label}</Text>
            </Pressable>
          );
        })}
      </ScrollView>
